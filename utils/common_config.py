...
def get_backbone(p):
    ...
    elif p['backbone'] == 'swin_base':
        from mmseg.models.backbones.swin import SwinTransformer
        backbone_channels = None
        ppath = './pretrained_ckpts/'
        backbone = SwinTransformer(patch_size=4, window_size=12, embed_dims=128, 
                    depths=(2, 2, 18, 2), num_heads=(4, 8, 16, 32), pretrain_img_size=p.TRAIN.SCALE,
                    pretrained=f'{ppath}/mmseg_swin_base_patch4_window12_384_22kto1k.pth')
        backbone.init_weights()
    else:
...
def get_model(p):
    ...
    if p['model'] == 'HiFAN':
        ...
        from models.HiFAN import HiFAN
        model = HiFAN(p, backbone)
    else:
...
def get_transformations(p):
    ...
    if p['train_db_name'] == 'NYUD' or p['train_db_name'] == 'PASCALContext':
        train_transforms = torchvision.transforms.Compose([ # from ATRC
            ...
            transforms.PhotoMetricDistortion(p=0.5),
...
def get_train_dataloader(p, dataset, sampler):
    ...
    collate = collate_mil
    assert p['trBatch'] % p['ACCUMULATION_STEPS'] == 0
    batch_size = p['trBatch'] // p['ACCUMULATION_STEPS']
    print(f"one forward batch_size: {batch_size}.\none gpu step batch_size: {p['trBatch']}.\n")
...
def get_loss(p, task=None):
    ...
    if task == 'edge':
        ...
        if 'label_smoothing' in p:
            ...
            label_smoothing = 0
        criterion = CrossEntropyLoss(ignore_index=p.ignore_index,label_smoothing=label_smoothing)
...
def get_criterion(p):
    ...
    elif p['loss_kwargs']['loss_scheme'] == 'loss_linear_decay':
        from losses.loss_schemes import MultiTaskLoss_linear_decay
        loss_ft = torch.nn.ModuleDict({task: get_loss(p, task) for task in p.TASKS.NAMES})
        loss_weights = p['loss_kwargs']['loss_weights']
        decays = p['loss_kwargs']['loss_decays']
        if 'inter_weight' in p:
            inter_weight = p['inter_weight']
        else:
            inter_weight = 1
        return MultiTaskLoss_linear_decay(p, p.TASKS.NAMES, loss_ft, loss_weights, decays, inter_weight=inter_weight)
    elif p['loss_kwargs']['loss_scheme'] == 'loss_log_linear_decay':
        from losses.loss_schemes import MultiTaskLoss_log_linear_decay
        loss_ft = torch.nn.ModuleDict({task: get_loss(p, task) for task in p.TASKS.NAMES})
        loss_weights = p['loss_kwargs']['loss_weights']
        decays = p['loss_kwargs']['loss_decays']
        return MultiTaskLoss_log_linear_decay(p, p.TASKS.NAMES, loss_ft, loss_weights, decays)
    else:
...
def get_optimizer(p, model):
    ...
    elif p['optimizer'] == 'sgd_momentum':
        optimizer = torch.optim.SGD(params, lr=p['optimizer_kwargs']['lr'], momentum=p['optimizer_kwargs']['momentum'], weight_decay=p['optimizer_kwargs']['weight_decay'])
    else:
